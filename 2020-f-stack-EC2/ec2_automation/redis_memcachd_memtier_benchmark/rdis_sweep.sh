...
#echo "./amd_memcached_core_scale.sh 1048576 127.0.0.1 1:0 inst${serv}${core} redis ${ms}-${me} /root/redis_memcachd_memtier_benchmark/core_scale ${stc1}-${edc1} ${port} "4" launchmemcache & "
if [ "$hypt" == "on" ]
then

	export hypt="on"
	echo "./amd_memcached_core_scale.sh 1048576 127.0.0.1 1:0 inst${serv}${core} redis ${ms}-${me} /root/redis_memcachd_memtier_benchmark/core_scale ${stc1}-${edc1},${stc2}-${edc2} ${port} "4" launchmemcache &"
	./amd_memcached_core_scale.sh 1048576 127.0.0.1 1:0 inst${serv}${core} redis ${ms}-${me} /root/redis_memcachd_memtier_benchmark/core_scale ${stc1}-${edc1},${stc2}-${edc2} ${port} "4" launchmemcache &

else
	 export hypt="off"

	echo "./amd_memcached_core_scale.sh 1048576 127.0.0.1 1:0 inst${serv}${core} redis ${ms}-${me} /root/redis_memcachd_memtier_benchmark/core_scale ${stc1}-${edc1} ${port} "4" launchmemcache &"
	./amd_memcached_core_scale.sh 1048576 127.0.0.1 1:0 inst${serv}${core} redis ${ms}-${me} /root/redis_memcachd_memtier_benchmark/core_scale ${stc1}-${edc1} ${port} "4" launchmemcache &
fi
#echo "./amd_memcached_core_scale.sh 1048576 127.0.0.1 1:0 inst${serv}${core} redis ${ms}-${me} /root/redis_memcachd_memtier_benchmark/core_scale ${stc1}-${edc1},${stc2}-${edc2} ${port} "4" launchmemcache &"
#./amd_memcached_core_scale.sh 1048576 127.0.0.1 1:0 inst${serv}${core} redis ${ms}-${me} /root/redis_memcachd_memtier_benchmark/core_scale ${stc1}-${edc1},${stc2}-${edc2} ${port} "4" launchmemcache &
wrpids[$serv]=$!
...
#./process_stats_of_file.py  --filepattern="inst*${core}_con${connections}_d1024.txt" --indexpattern="GET" --statpos=1 --indexpos=0 --outfile=testsum
./process_stats_of_file.py  --filepattern="inst[0-9]${core}_con${connections}_d1024.txt" --indexpattern="GET" --statpos=1 --indexpos=0 --outfile=testsum
...
hpthrput=$(cat inst[0-9]${core}_con${connections}_d1024.txt  | grep "Totals" | awk '{sumt += $2} END {print sumt}')
...
mkdir -p $res_dir
if [ ! -f $res_dir/d1k_instance_run_sweep.txt ]
then
	touch $res_dir/d1k_instance_run_sweep.txt
fi

echo "${stc1}-${edc1} ${total} ${ins} ${connections} ${reshp} ${hpthrput} ${hypt}" >> $res_dir/d1k_instance_run_sweep.txt
