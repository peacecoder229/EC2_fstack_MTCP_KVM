...
sleep 3
		...
		for act in "build"
			...
			sleep 1
			./run_client_and_server_memtier_r1p1_emon.py --ratio ${ratio}  --scores=${core} --mark ip6_${act}_d${d} --host ${host} --ccores=0-7 --port=all  --keymax=1000001 --con_num=$con -n $num -p $p -d $d --sport 9001 --proto redis
