...
if __name__ == "__main__":
    ...
    for f in allfiles:
        with open(f, "r") as out:
            for line in out:
                line = line.split()
                # Below if any of the item in items matche with line then it returns true and that itm which matches the line.
                #It wirks here becuase only one of the item is supposed to be in the line
                itemfound = checkIfinLookup(items, line)
                if itemfound:
        ...
        #below adding new columns util, ipc and scyc etc.. for adding rows we could do df.loc[rowname]= pd_series
