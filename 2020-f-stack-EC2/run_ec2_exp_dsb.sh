...
then
  echo "
        ...
        Usage: ./run_ec2_exp_dsb.sh ProcessorType(intel/amd) InstanceType(e.g. c5.12xlarge, c5.4xlarge) NoOfCPUs(e.g. 8,12,24) CaseNo(case_1/case_2) VMType(primary/secondary) Mode(init/run) InstanceFile RunNo(1,2 ... etc.).
        "
...
create_ec2() {
  ...
  local start_up_script='file://startup_script_dsb.sh'
  ...
  --user-data file://./startup_script_nginx.sh | jq -r '.Instances[0].InstanceId')
...
scp_exp_script() {
  ...
  local copy_dir='ec2_dsb_exp_dir'
  local remote_dir='/home/ubuntu/'
  
  #sed -i "s/worker_processes  1/worker_processes  $n_worker_procs/" ./ec2_nginx_exp_dir/nginx.conf
...
run_exp_script() {
  ...
  local network_tune_command="sudo sysctl -w net.ipv4.tcp_max_syn_backlog=6553600 && sudo sysctl -w net.core.wmem_max=838860800 && sudo sysctl -w net.core.rmem_max=838860080 && sudo sysctl -w net.core.somaxconn=51200 && sudo sysctl -w net.core.optmem_max=8192000 && ulimit -n 80000"
  local chmod_command="export TZ=America/Los_Angeles && pushd /home/ubuntu/ec2_nginx_exp_dir && sudo chmod +x *.sh && popd"
  local exp_script="sudo bash /home/ubuntu/ec2_nginx_exp_dir/ec2_nginx_exp_script.sh $case_no $vm_type $instance_id $run_no $no_of_cores > /home/ubuntu/output.log"
  
  local full_command="$network_tune_command && $chmod_command && $exp_script"
...
scp_exp_result() {
  ...
  #local local_dir="./ec2_exp_nginx_drc_res/$instance_processor/$instance_type"
  local local_dir="./ec2_exp_nginx_drc_res/$instance_processor/"
