...
then
	...
	echo "./amd_memcached_core_scale.sh 1048576 127.0.0.1 1:4 inst${serv}${core} memcache_text 24-${cedc1}  /root/redis_memcachd_memtier_benchmark/core_scale ${stc1}-${edc1} ${port} ${connections} &"
	./amd_memcached_core_scale.sh 1048576 127.0.0.1 1:4 inst${serv}${core} memcache_text 24-${cedc1}  /root/redis_memcachd_memtier_benchmark/core_scale ${stc1}-${edc1} ${port} ${connections} &
...
mkdir -p $res_dir
if [ ! -f $res_dir/memc_load_test.txt ]
then
	touch $res_dir/memc_load_test.txt
fi

echo "${total} ${ins} ${connections} ${reshp} ${hpthrput} ${hypt}" >> $res_dir/memc_load_test.txt
