...
#Note:This script is for 24 cores.  for different no of cpus change the value of ms, me (line 70,71) and ci, cj(line 130)
...
# client cores
ms=12 # server runs in 0-11 cores, so use the next ones for client
me=13
...
#echo "./amd_memcached_core_scale.sh 1048576 127.0.0.1 1:0 inst${serv}${core} $protocol ${ms}-${me} /home/ubuntu/redis_memcachd_memtier_benchmark/core_scale ${stc1}-${edc1} ${port} "4" launchmemcache & "
if [ "$hypt" == "on" ]
then

	export hypt="on"
	echo "./amd_memcached_core_scale.sh 1048576 127.0.0.1 1:0 inst${serv}${core} $protocol ${ms}-${me} /home/ubuntu/redis_memcachd_memtier_benchmark/core_scale ${stc1}-${edc1},${stc2}-${edc2} ${port} "4" ${launchserv} &"
	./amd_memcached_core_scale.sh 1048576 127.0.0.1 1:0 inst${serv}${core} $protocol ${ms}-${me} /home/ubuntu/redis_memcachd_memtier_benchmark/core_scale ${stc1}-${edc1},${stc2}-${edc2} ${port} "4" ${launchserv} &

else
	export hypt="off"

	echo "./amd_memcached_core_scale.sh 1048576 127.0.0.1 1:0 inst${serv}${core} $protocol ${ms}-${me} /home/ubuntu/redis_memcachd_memtier_benchmark/core_scale ${stc1}-${edc1} ${port} "4" ${launchserv} &"
	./amd_memcached_core_scale.sh 1048576 127.0.0.1 1:0 inst${serv}${core} $protocol ${ms}-${me} /home/ubuntu/redis_memcachd_memtier_benchmark/core_scale ${stc1}-${edc1} ${port} "4" ${launchserv} &
fi
#echo "./amd_memcached_core_scale.sh 1048576 127.0.0.1 1:0 inst${serv}${core} $protocol ${ms}-${me} /home/ubuntu/redis_memcachd_memtier_benchmark/core_scale ${stc1}-${edc1},${stc2}-${edc2} ${port} "4" launchmemcache &"
#./amd_memcached_core_scale.sh 1048576 127.0.0.1 1:0 inst${serv}${core} $protocol ${ms}-${me} /home/ubuntu/redis_memcachd_memtier_benchmark/core_scale ${stc1}-${edc1},${stc2}-${edc2} ${port} "4" launchmemcache &
wrpids[$serv]=$!
serv=$(( serv+1 ))
...
serv=0
port=9000
ci=12 # server runs in 0-11 cores, so the clients will run from 12-23.
cj=23
...
cd /home/ubuntu/redis_memcachd_memtier_benchmark/core_scale;  rm -rf inst*; rm -rf memtier_ip*; sleep 1;
...
then
	...
	echo "./amd_memcached_core_scale.sh 1048576 127.0.0.1 1:4 inst${serv}${core} $protocol ${cstc1}-${cedc1},${cstc2}-${cedc2}  /home/ubuntu/redis_memcachd_memtier_benchmark/core_scale ${stc1}-${edc1},${stc2}-${edc2} ${port} ${connections} &"
	./amd_memcached_core_scale.sh 1048576 127.0.0.1 1:4 inst${serv}${core} $protocol ${cstc1}-${cedc1},${cstc2}-${cedc2}  /home/ubuntu/redis_memcachd_memtier_benchmark/core_scale ${stc1}-${edc1},${stc2}-${edc2} ${port} ${connections} &

else

	echo "./amd_memcached_core_scale.sh 1048576 127.0.0.1 1:4 inst${serv}${core} $protocol ${cstc1}-${cedc1}  /home/ubuntu/redis_memcachd_memtier_benchmark/core_scale ${stc1}-${edc1} ${port} ${connections} &"
	./amd_memcached_core_scale.sh 1048576 127.0.0.1 1:4 inst${serv}${core} $protocol ${cstc1}-${cedc1}  /home/ubuntu/redis_memcachd_memtier_benchmark/core_scale ${stc1}-${edc1} ${port} ${connections} &
...
cd /home/ubuntu/redis_memcachd_memtier_benchmark/core_scale
...
p_50=$(awk -F, '{print $7}' testsum | tail -1)
p_75=$(awk -F, '{print $8}' testsum | tail -1)
p_90=$(awk -F, '{print $9}' testsum | tail -1)
p_10=$(awk -F, '{print $10}' testsum | tail -1)
p_20=$(awk -F, '{print $11}' testsum | tail -1)
p_30=$(awk -F, '{print $12}' testsum | tail -1)
p_40=$(awk -F, '{print $13}' testsum | tail -1)
p_60=$(awk -F, '{print $14}' testsum | tail -1)
...
res_dir_name=${res_dir}_${exp_time}

mkdir -p $res_dir_name
if [ ! -f $res_dir_name/${type}d1k_instance_run_sweep.txt ]
then
	touch $res_dir_name/${type}d1k_instance_run_sweep.txt
fi

#echo "${stc1}-${edc1} ${total} ${ins} ${connections} ${reshp} ${hpthrput} ${hypt}" >> $res_dir_name/${type}d256b_instance_run_sweep.txt

echo "${exp_time}, ${stc1}-${edc1}, ${total}, ${ins}, ${connections} ,${min}, ${max}, ${sumavg}, ${p_10}, ${p_20}, ${p_30}, ${p_40}, ${p_50}, ${p_60}, ${p_75}, ${p_90}, ${hpthrput}, ${hypt}" >> $res_dir_name/${type}_d256b_instance_run_sweep.txt
