...
from perfkitbenchmarker.linux_benchmarks import intel_hpc_blacksholes_benchmark
...
class HPCBlackSholesBenchmarkTest(unittest.TestCase):
  ...
  def testParseResult(self):
    ...
    blacksholes_name = intel_hpc_blacksholes_benchmark.__name__
    samples = []
    with mock.patch(blacksholes_name + '.intel_hpc_utils.RunSysinfo'), \
        mock.patch(blacksholes_name + '.intel_hpc_utils.GetNodeArchives'):
      benchmark_spec.vms[0].RemoteCommand.side_effect = [('', ''), ('FOM in seconds : 124.80131873952067746004', '')]
      samples = intel_hpc_blacksholes_benchmark.Run(benchmark_spec)
