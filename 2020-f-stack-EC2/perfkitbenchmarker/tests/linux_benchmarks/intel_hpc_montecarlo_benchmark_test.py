...
from perfkitbenchmarker.linux_benchmarks import intel_hpc_montecarlo_benchmark
...
class HPCMonteCarloBenchmarkTest(unittest.TestCase):
  ...
  def testParseResult(self):
    ...
    montecarlo_name = intel_hpc_montecarlo_benchmark.__name__
    samples = []
    with mock.patch(montecarlo_name + '.intel_hpc_utils.RunSysinfo'), \
        mock.patch(montecarlo_name + '.intel_hpc_utils.GetNodeArchives'):
      benchmark_spec.vms[0].RemoteCommand.side_effect = [('', ''), ('FOM in seconds : 124.80131873952067746004', '')]
      samples = intel_hpc_montecarlo_benchmark.Run(benchmark_spec)
