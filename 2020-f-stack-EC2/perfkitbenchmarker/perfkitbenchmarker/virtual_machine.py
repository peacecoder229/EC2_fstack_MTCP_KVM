...
flags.DEFINE_boolean(
    ...
    'addresses.')
flags.DEFINE_string(
    'proxy_server_ip_list', './perfkitbenchmarker/data/proxy_ip_list/proxy_ip_list.txt',
    'A list of proxy server IP addresses in plain text, with one cidr per line. '
    'This is used to add inbound rules for SSH port 22 to the security group associated '
    'to the VM(s). Rename or remove the file to ignore the list. '
    'If ignored or the file is empty, however, cidr 0.0.0.0/0 inbound rule '
    'will be used for the security group, meaning port 22 is open to any incoming IP. '
    'The latter exposes a security vunerability to the VM(s). '
    'This list adds constrains for inbound traffic from Internet only; Internal '
    'communications among the VM(s) in the same VPC or subnet is not impacted. '
    'However, communications between VPCs has not been studied.')
...
class BaseVirtualMachine(BaseOsMixin, resource.BaseResource):
  ...
  def __init__(self, vm_spec):
    ...
    # Cumulus addition - hopefully this comment prevents silent removal in rebase
    self.additional_private_ip_addresses_count = vm_spec.additional_private_addresses_count
    self.additional_private_ip_addresses = []
    # end Cumulus addition
  ...
  def InsertProxyIPList(self):
    ...
    source_range = []
    file_path = FLAGS.proxy_server_ip_list
    absPath = os.path.abspath(os.path.expanduser(file_path))
    if not os.path.isfile(absPath):
      logging.info('file ({}) does not exist'.format(absPath))
      return None
