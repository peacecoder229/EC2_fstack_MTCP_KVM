...
from perfkitbenchmarker import offline_util
...
class BaseRhelMixin(BaseLinuxMixin):
  ...
  def OfflineModePrepare(self):
    CaKey = os.path.expanduser(FLAGS.offline_cakey)
    TmpDest = "mitmproxy-ca-cert.crt"
    RemoteDest = "/etc/pki/ca-trust/source/anchors"
    self.RemoteCopy(CaKey, TmpDest, True)
    self.RemoteCommand("sudo mkdir -p {}".format(RemoteDest))
    self.RemoteCommand("if [[ ! -f {1} ]]; then sudo cp {0} {1}; fi"
                       .format(TmpDest, RemoteDest))
    self.RemoteCommand("sudo update-ca-trust")
    CertFile = "/etc/pki/ca-trust/extracted/pem/tls-ca-bundle.pem"
    PipConf = "/etc/pip.conf"
    # If pip.conf doesn't exist, create one with our proxy info
    cmd = [
        "if [[ ! -f {0} ]]; then".format(PipConf),
        "echo '[global]\ncert = {0}\ntrusted-host = {1}\nproxy = {2}'"
        .format(CertFile, FLAGS.offline_listen_ipaddr, FLAGS.http_proxy),
        " | sudo tee -a {0};fi".format(PipConf)
    ]
...
class BaseDebianMixin(BaseLinuxMixin):
  ...
  def RestorePackages(self):
    ...
    def _PurgePackages(current_packages, initial_package_names):
      ...
      remove_packages = 'sudo apt-get --purge remove -y'
      package_idx = 0
      commands_list = []
      checked_packages = {}
      ...
      while package_idx < len(current_packages):
        ...
        for count in range(MAX_PACKAGE_PER_CMD):
          if package_idx < len(current_packages):
            ...
            if package not in unavailable_package_names:
              cmd += " " + package
              package_idx += 1
          else:
            break
      ...
      for madison_pack in commands_list:
        ...
        for line in check_package:
          package = re.search(madison_package_infos, line).group(1)
          version = re.search(madison_package_infos, line).group(2)
          architecture = re.search(madison_package_infos, line).group(3)
          package_pair = (version, architecture)
      ...
      for element in reversed(current_packages):
        ...
        if validate_package is False:
          ...
          validate_package = False
      self.RemoteCommand(remove_packages)
    ...
    for pack in commands_list:
      self.RemoteCommand(pack)
    self.RemoteCommand('sudo apt-get -y autoremove')
  ...
  def OfflineModePrepare(self):
    """ Install mitmproxy key and configure pip to use it """
    CaKey = os.path.expanduser(FLAGS.offline_cakey)
    self.Install('pip')
    # To avoid changing perms on the final dest
    CaConf = "/etc/ca-certificates.conf"
    TmpDest = "mitmproxy-ca-cert.crt"
    RemoteDest = "/usr/share/ca-certificates/extra"
    self.RemoteCopy(CaKey, TmpDest, True)
    self.RemoteCommand("sudo mkdir -p {}".format(RemoteDest))
    self.RemoteCommand("if [[ ! -f {1} ]]; then sudo cp {0} {1}; fi"
                       .format(TmpDest, RemoteDest))
    cmd = [
        "if [[ ! `grep {0} {1}` ]];".format(TmpDest, CaConf),
        "then echo 'extra/mitmproxy-ca-cert.crt' |",
        " sudo tee -a {0}; fi".format(CaConf)
    ]
    self.RemoteCommand(' '.join(cmd))
    self.RemoteCommand("sudo update-ca-certificates")
    CertFile = "/usr/lib/ssl/certs/ca-certificates.crt"
    cmd = [
        "if [[ -f {0} ]] && [[ ! `grep '#mitmproxy' {0}` ]];".format(CertFile),
        "then echo '#mitmproxy' | cat - {0} |".format(TmpDest),
        "sudo tee -a {0}; fi".format(CertFile)
    ]
    self.RemoteCommand(" ".join(cmd))
    CertFiles = [
    ...
    cmd = [
        "if [[ ! -f {0} ]]; then".format(PipConf),
        "echo '[global]\ncert = {0}\n".format(CertFile),
        "trusted-host = {0}\n".format(FLAGS.offline_listen_ipaddr),
        "proxy = {0}\n'".format(FLAGS.http_proxy),
        " | sudo tee -a {0};fi".format(PipConf)
    ]
    self.RemoteCommand(" ".join(cmd))
...
class Ubuntu1804Mixin(BaseUbuntuMixin):
  ...
  def UpdateEnvironmentPath(self):
    ...
    """
    self.RemoteCommand(
        r'touch ~/.bashrc && sudo sed -i "1 i\export PATH=$PATH:/snap/bin" ~/.bashrc')
    self.RemoteCommand(
        r'sudo touch /etc/bash.bashrc && sudo sed -i "1 i\export PATH=$PATH:/snap/bin" /etc/bash.bashrc')


class Ubuntu1910Mixin(BaseUbuntuMixin):
  """Class holding Ubuntu1910 specific VM methods and attributes."""
  OS_TYPE = os_types.UBUNTU1910
