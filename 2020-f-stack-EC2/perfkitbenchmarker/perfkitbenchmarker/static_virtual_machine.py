...
class StaticVmSpec(virtual_machine.BaseVmSpec):
  ...
  def __init__(self, component_full_name, ip_address=None, user_name=None,
               ...
               zone=None, additional_ips=None, **kwargs):
    ...
    Args:
      ...
      zone: The VM's zone.
      additional_ips: The VM's additional private ips
...
class StaticVirtualMachine(virtual_machine.BaseVirtualMachine):
  ...
  def __init__(self, vm_spec):
    ...
    self.additional_ips = vm_spec.additional_ips
  ...
  def ReadStaticVirtualMachineFile(cls, file_obj):
    ...
    optional_keys = frozenset(['internal_ip', 'zone', 'local_disks',
                               ...
                               'ssh_port', 'install_packages', 'additional_ips'])
    ...
    for item in vm_arr:
      ...
      additional_ips = item.get('additional_ips')
      ...
      vm_spec = StaticVmSpec(
          ...
          flag_values=flags.FLAGS, additional_ips=additional_ips)
  ...
  def CreateAdditionalPrivateIpAddresses(self):
    ...
    """
    out, _ = self.RemoteCommand("ip -o addr show scope global | "
                                "awk '{print $4}' | cut -f1 -d '/'")
    private_addresses = [addr for addr in str(out.strip()).splitlines()
                         if addr != self.internal_ip]
    private_addresses_len = len(private_addresses)
    if private_addresses_len:
      if self.additional_ips:
        self.additional_private_ip_addresses = [ip.strip() for ip in self.additional_ips.split(",")
                                                if ip.strip() in private_addresses]
      elif self.additional_private_ip_addresses_count:
        if private_addresses_len > self.additional_private_ip_addresses_count:
          logging.info("{0} has more ip addresses than the requested value, "
                       "using {1}".format(self.name, self.additional_private_ip_addresses_count))
          private_addresses = private_addresses[:self.additional_private_ip_addresses_count]
        elif private_addresses_len < self.additional_private_ip_addresses_count:
          logging.info("{0} has less ip addresses than the requested value, "
                       "using only {1}".format(self.name, private_addresses_len))
        self.additional_private_ip_addresses = private_addresses[:]
...
class Ubuntu1804BasedStaticVirtualMachine(StaticVirtualMachine,
                                          linux_virtual_machine.Ubuntu1804Mixin):
  pass


class Ubuntu1910BasedStaticVirtualMachine(StaticVirtualMachine,
                                          linux_virtual_machine.Ubuntu1910Mixin):
  pass


class Ubuntu2004BasedStaticVirtualMachine(StaticVirtualMachine,
