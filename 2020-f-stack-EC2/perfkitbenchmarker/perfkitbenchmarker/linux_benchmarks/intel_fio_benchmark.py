...
from perfkitbenchmarker.linux_packages import intel_fio as fio
import six
...
flags.DEFINE_string('intel_fio_jobfile', None,
                    'Job file that fio will use. If not given, use a job file '
                    'bundled with PKB. Cannot use with '
                    '--intel_fio_generate_scenarios.')
flags.DEFINE_list('intel_fio_generate_scenarios', [],
                  ...
                  '--intel_fio_jobfile.')
flags.DEFINE_enum('intel_fio_target_mode', AGAINST_FILE_WITHOUT_FILL_MODE,
                  ...
                  'to prefill.')
flags.DEFINE_string('intel_fio_fill_size', '100%',
                    ...
                    '--intel_fio_target_mode is against_device_with_fill or '
                    'against_file_with_fill.')
flag_util.DEFINE_integerlist('intel_fio_io_depths', flag_util.IntegerList([1]),
                             'IO queue depths to run on. Can specify a single '
                             'number, like --intel_fio_io_depths=1, a range, like '
                             '--intel_fio_io_depths=1-4, or a list, like '
                             '--intel_fio_io_depths=1-4,6-8',
                             on_nonincreasing=flag_util.IntegerListParser.WARN,
                             module_name=__name__)
flag_util.DEFINE_integerlist('intel_fio_num_jobs', flag_util.IntegerList([1]),
                             'Number of concurrent fio jobs to run.',
                             on_nonincreasing=flag_util.IntegerListParser.WARN,
                             module_name=__name__)
flags.DEFINE_integer('intel_fio_working_set_size', None,
                     'The size of the working set, in GB. If not given, use '
                     'the full size of the device. If using '
                     '--intel_fio_generate_scenarios and not running against a raw '
                     'device, you must pass --intel_fio_working_set_size.',
                     lower_bound=0)
flag_util.DEFINE_units('intel_fio_blocksize', None,
                       'The block size for fio operations. Default is given by '
                       'the scenario when using --generate_scenarios. This '
                       'flag does not apply when using --intel_fio_jobfile.',
                       convertible_to=units.byte)
flags.DEFINE_integer('intel_fio_runtime', 600,
                     'The number of seconds to run each fio job for.',
                     lower_bound=1)
flags.DEFINE_list('intel_fio_parameters', ['randrepeat=0'],
                  'Parameters to apply to all PKB generated fio jobs. Each '
                  'member of the list should be of the form "param=value".')
flags.DEFINE_boolean('intel_fio_lat_log', False,
                     'Whether to collect a latency log of the fio jobs.')
flags.DEFINE_boolean('intel_fio_bw_log', False,
                     'Whether to collect a bandwidth log of the fio jobs.')
flags.DEFINE_boolean('intel_fio_iops_log', False,
                     'Whether to collect an IOPS log of the fio jobs.')
flags.DEFINE_integer('intel_fio_log_avg_msec', 1000,
                     ...
                     lower_bound=0)
flags.DEFINE_boolean('intel_fio_hist_log', False,
                     'Whether to collect clat histogram.')
flags.DEFINE_integer('intel_fio_log_hist_msec', 1000,
...
flags.DEFINE_boolean(  # TODO(user): Add support for simultaneous read.
    'intel_fio_write_against_multiple_clients', False,
    'Whether to run fio against multiple nfs. Only applicable '
    'when running fio against network mounts and rw=write.')
flags.DEFINE_integer('intel_fio_command_timeout_sec', None,
                     'Timeout for fio commands in seconds.')


FLAGS_IGNORED_FOR_CUSTOM_JOBFILE = {
    'intel_fio_generate_scenarios', 'intel_fio_io_depths', 'intel_fio_runtime',
    'intel_fio_blocksize', 'intel_fio_num_jobs', 'intel_fio_parameters'}
...
def AgainstDevice():
  ...
  """
  return FLAGS.intel_fio_target_mode in AGAINST_DEVICE_MODES
...
def FillTarget():
  ...
  """
  return FLAGS.intel_fio_target_mode in FILL_TARGET_MODES
...
def WarnOnBadFlags():
  ...
  if (FLAGS.intel_fio_jobfile is None and
      FLAGS.intel_fio_generate_scenarios and
      not FLAGS.intel_fio_working_set_size and
      not AgainstDevice()):
...
def GetConfig(user_config):
  ...
  if FLAGS.intel_fio_target_mode != AGAINST_FILE_WITHOUT_FILL_MODE:
...
def GetLogFlags(log_file_base):
  """Gets fio log files."""
  collect_logs = FLAGS.intel_fio_lat_log or FLAGS.intel_fio_bw_log or FLAGS.intel_fio_iops_log
  fio_log_flags = [(FLAGS.intel_fio_lat_log, '--write_lat_log=%(filename)s',),
                   (FLAGS.intel_fio_bw_log, '--write_bw_log=%(filename)s',),
                   (FLAGS.intel_fio_iops_log, '--write_iops_log=%(filename)s',),
                   (FLAGS.intel_fio_hist_log, '--write_hist_log=%(filename)s',),
                   (collect_logs, '--log_avg_msec=%(interval)d',)]
  fio_command_flags = ' '.join([flag for given, flag in fio_log_flags if given])
  if FLAGS.intel_fio_hist_log:
    fio_command_flags = ' '.join([
        fio_command_flags, '--log_hist_msec=%(hist_interval)d'])

  return fio_command_flags % {'filename': log_file_base,
                              'interval': FLAGS.intel_fio_log_avg_msec,
                              'hist_interval': FLAGS.intel_fio_log_hist_msec}
...
def PrepareWithExec(vm, exec_path):
  ...
  if FillTarget():
    ...
    FillDevice(vm, disk, FLAGS.intel_fio_fill_size, exec_path)
  ...
  if FLAGS.intel_fio_target_mode == AGAINST_FILE_WITH_FILL_MODE:
  ...
  if FLAGS.intel_fio_write_against_multiple_clients:
...
def RunWithExec(vm, exec_path, remote_job_file_path, job_file_contents):
  ...
  job_file_string = GetOrGenerateJobFileString(
      FLAGS.intel_fio_jobfile,
      FLAGS.intel_fio_generate_scenarios,
      AgainstDevice(),
      disk,
      FLAGS.intel_fio_io_depths,
      FLAGS.intel_fio_num_jobs,
      FLAGS.intel_fio_working_set_size,
      FLAGS.intel_fio_blocksize,
      FLAGS.intel_fio_runtime,
      FLAGS.intel_fio_parameters,
      job_file_contents)
  ...
  collect_logs = any([FLAGS.intel_fio_lat_log, FLAGS.intel_fio_bw_log, FLAGS.intel_fio_iops_log,
                      FLAGS.intel_fio_hist_log])
  ...
  stdout, _ = vm.RobustRemoteCommand(
      fio_command, should_log=True, timeout=FLAGS.intel_fio_command_timeout_sec)
  end_time = time.time()
  bin_vals = []
  if collect_logs:
  ...
  nvme_cli_version, _ = vm.RemoteCommand('nvme --version', retries=1, ignore_failure=True)
  gcc_version, _ = vm.RemoteCommand('gcc --version | head -n 1', retries=1, ignore_failure=True)
  git_commit_id, _ = vm.RemoteCommand('cd {0} && git log --pretty=format:\'%H\' -n 1'.format(fio.FIO_DIR))

  for item in samples:
    item.metadata['fio_repository_url'] = FLAGS.intel_fio_repository
    item.metadata['fio_version'] = FLAGS.intel_fio_version
    item.metadata['fio_git_commit_id'] = git_commit_id
    item.metadata['nvm_cli_version'] = nvme_cli_version.strip()
    item.metadata['gcc_version'] = gcc_version.strip()
...
def Cleanup(benchmark_spec):
  ...
  if not AgainstDevice() and not FLAGS.intel_fio_jobfile:
