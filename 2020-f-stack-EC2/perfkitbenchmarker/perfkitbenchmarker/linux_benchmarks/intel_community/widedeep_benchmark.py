...
INSTALLED_PACKAGES = {}
OS_INFO = {}
BENCHMARK_NAME = "widedeep"
BENCHMARK_DATA_DIR = "widedeep_benchmark"
...
flags.DEFINE_integer("widedeep_vcpu", None, "Number of virtual cpus to use. ")
flags.DEFINE_enum("widedeep_run_mode", "default", ('default', 'custom', 'optimize'), "Select default, custom, or optimize. ")
flags.DEFINE_string("widedeep_benchmark_mode", "benchmark-only", "Select either benchmark-only or accuracy to use. ")
flags.DEFINE_string("widedeep_framework", "tensorflow-1.14.1", "Version of tensorflow framework to use. ")
flags.DEFINE_enum("widedeep_precision", "fp32", ('int8', 'fp32'), "Inference precision to use. ")
flags.DEFINE_integer("widedeep_batchsize", 1, "Batch size to use. ")
flags.DEFINE_integer("widedeep_intra_threads", 1, "Number of intra-threads to use. ")
flags.DEFINE_integer("widedeep_inter_threads", 1, "Number of inter-threads to use. ")
flags.DEFINE_integer("widedeep_omp_threads", 1, "Number of omp threads to use. ")
...
def _InitFlags(benchmark_spec):
  if not FLAGS['widedeep_vcpu'].present:
...
def Prepare(benchmark_spec):
  ...
  if not isinstance(target, list):
    target = [target]
  res = ['"https://cumulus.s3.us-east-2.amazonaws.com/wide_n_deep/model.tar.gz"', '"https://cumulus.s3.us-east-2.amazonaws.com/wide_n_deep/widedeep_wlfiles.tar.gz"']
  ...
  if isinstance(target, list):
    vm_util.RunThreaded(lambda vm: vm.RemoteCommand("cd {0} && ".format(BENCHMARK_DIR) + "./install_widedeep_wl.bash "), target)
  else:
    target.RemoteCommand("cd {0} && ".format(BENCHMARK_DIR) + "./install_widedeep_wl.bash ")
...
def Run(benchmark_spec):
  ...
  run_cmd = "cd {0} && ".format(BENCHMARK_DIR) + "./measure_inference_performance.bash --run_mode={0} --benchmark_mode={1} --vcpu={2} --framework={3} --precision={4} --batchsize={5} --intra_threads={6} --inter_threads={7} --omp_threads={8}".format(FLAGS.widedeep_run_mode, FLAGS.widedeep_benchmark_mode, FLAGS.widedeep_vcpu, FLAGS.widedeep_framework, FLAGS.widedeep_precision, FLAGS.widedeep_batchsize, FLAGS.widedeep_intra_threads, FLAGS.widedeep_inter_threads, FLAGS.widedeep_omp_threads)
  vm_group_vm_group1[0].RemoteCommand(run_cmd)
  metadata["goal"] = "Increase"
  vm = vm_group_vm_group1[0]
  out, _ = vm.RemoteCommand("cat {0}/benchmark_wide_deep_large_ds_inference_*.log | grep 'Throughput is' | awk -F':' '{{ print $2 }}'".format("/opt/pkb/results"))
  val = out.strip()
  results.append(sample.Sample("Throughput ", val, "records/sec", metadata))
  vm_group_vm_group1[0].RemoteCopy(vm_util.GetTempDir(), "/opt/pkb/results", False)
  return results
...
def Cleanup(benchmark_spec):
  ...
  target = vm_group_vm_group1[0]
  if isinstance(target, list):
    vm_util.RunThreaded(lambda vm: vm.RemoteCommand("echo cleaning up work directories; rm -rf ~/wide_deep_large_ds"), target)
  else:
    target.RemoteCommand("echo cleaning up work directories; rm -rf ~/wide_deep_large_ds")
