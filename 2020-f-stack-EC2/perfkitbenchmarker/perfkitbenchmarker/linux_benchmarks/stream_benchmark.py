...
def _CreateMetadataFromFlags():
  """Create meta data with all flags for STREAM."""
  metadata = {
      'stream_array_size': FLAGS.stream_array_size,
      'stream_ntimes': FLAGS.stream_ntimes,
      'stream_offset': FLAGS.stream_offset,
      'stream_compiler_flags': FLAGS.stream_compiler_flags,
      'stream_omp_num_threads': FLAGS.stream_omp_num_threads
  }
  return metadata
...
def _ParseStreamOutput(stream_output, metadata):
  ...
  for line in stream_output_io:
    ...
    if sline.startswith("STREAM version"):
      metadata['STREAM version'] = str(_GetFloatFromLine(sline))
    elif sline.startswith("Memory per array"):
      metadata['Memory per array'] = str(_GetFloatFromLine(sline)) + " MiB"
    elif sline.startswith("Total memory"):
      metadata['Total memory required'] = str(_GetFloatFromLine(sline)) + " MiB"
      ...
      if sline.startswith("Triad:"):
        tbr = rate
      if FLAGS.stream_iterations >= 3 and function == "Triad":
        warning_metadata = copy.deepcopy(metadata)
        warning_metadata.update({"WARNING": "In some cases reporting median of best 3 runs is equivalent to cherry picking the result."})
        results.append(sample.Sample(function + " Best Rate", rate, "MB/s", warning_metadata))
      else:
        results.append(sample.Sample(function + " Best Rate", rate, "MB/s", metadata))
      results.append(sample.Sample(function + " Avg time", avg_time, "s", metadata))
      results.append(sample.Sample(function + " Min time", min_time, "s", metadata))
      results.append(sample.Sample(function + " Max time", max_time, "s", metadata))
  return tbr, results
...
def Run(benchmark_spec):
  ...
  if FLAGS.stream_iterations == 0:
    ...
    results.append(sample.Sample("RSD for all {} iterations".format(FLAGS.stream_iterations), rsd, "%", metadata))
  else:
  ...
  if FLAGS.stream_binary_url:
    metadata['stream_binary'] = '{}'.format(FLAGS.stream_binary_url)
  else:
    metadata['stream_repo'] = stream.GIT_REPO
    metadata['compiler_version'] = compiler_version.strip()
