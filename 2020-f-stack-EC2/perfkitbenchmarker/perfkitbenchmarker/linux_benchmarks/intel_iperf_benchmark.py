...
flags.DEFINE_integer('intel_iperf_sending_thread_count', 1,
                     'Number of connections to make to the '
                     'server for sending traffic.',
                     lower_bound=1)
flags.DEFINE_integer('intel_iperf_runtime_in_seconds', 60,
                     'Number of seconds to run iperf.',
                     lower_bound=1)
flags.DEFINE_integer('intel_iperf_timeout', None,
...
def _RunIperf(sending_vm, receiving_vm, receiving_ip_address, ip_type):
  ...
  iperf_cmd = ('iperf --client %s --port %s --format m --time %s -P %s' %
               (receiving_ip_address, IPERF_PORT,
                FLAGS.intel_iperf_runtime_in_seconds,
                FLAGS.intel_iperf_sending_thread_count))
  ...
  timeout_buffer = FLAGS.intel_iperf_timeout or 30 + FLAGS.intel_iperf_sending_thread_count
  stdout, _ = sending_vm.RemoteCommand(iperf_cmd, should_log=True,
                                       timeout=FLAGS.intel_iperf_runtime_in_seconds +
                                       timeout_buffer)
  ...
  if not thread_values:
    ...
    if len(thread_values) != FLAGS.intel_iperf_sending_thread_count:
      raise ValueError('Only %s out of %s iperf threads reported a'
                       ' throughput value.' %
                       (len(thread_values), FLAGS.intel_iperf_sending_thread_count))

  total_throughput = 0.0
  for value in thread_values:
    total_throughput += float(value)

  # iperf --version exits 1 under normal conditions, with version in stderr
  _, sending_iperf_version = sending_vm.RemoteCommand('iperf --version', retries=1, ignore_failure=True)
  ...
  metadata = {
      ...
      'sending_thread_count': FLAGS.intel_iperf_sending_thread_count,
      'sending_zone': sending_vm.zone,
      'runtime_in_seconds': FLAGS.intel_iperf_runtime_in_seconds,
      'ip_type': ip_type,
      'sending_iperf_version': sending_iperf_version.strip()
  }
