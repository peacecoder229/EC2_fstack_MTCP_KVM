...
flags.DEFINE_string('no_proxy', '',
                    'Specify host(s) to exclude from proxy.'
                    'example: --no_proxy=192.168.0.1')
...
def _InitOfflineMode(offlineProxySvc):
  if FLAGS.offline_executable:
    if FLAGS.offline_mode == "record":
      offlineProxySvc.StartRecordMode()
    elif FLAGS.offline_mode == "replay":
      offlineProxySvc.StartReplayMode()
    elif FLAGS.offline_mode == "log":
      offlineProxySvc.StartUpstreamMode()
    else:
      logging.error("Unknown offline proxy mode. Not starting offline proxy.")
      raise Exception("Unknown mode. Please Check your offline mode flags.")
  # Override proxy flags
  FLAGS.http_proxy = offlineProxySvc.GetProxyString()
  FLAGS.https_proxy = offlineProxySvc.GetProxyString()
  logging.info("Using offline proxy at " + offlineProxySvc.GetProxyString())


def _CleanupOfflineMode(offlineProxySvc):
  """ Execute final and cleanup tasks """
  if FLAGS.offline_mode == "record":
    # Create a manifest with checksum
    Manifest = offlineProxySvc.WriteManifest()
    # Verify the checksum is valid
    if not offlineProxySvc.VerifyChecksum(Manifest):
      logging.error("Cache checksum verification failed!")
    else:
      logging.info("Cache checksum verification succeeded!")
    # Pretty print manifest
  offlineProxySvc.Stop()
...
def DoPreparePhase(spec, timer):
  ...
  events.before_phase.send(events.PREPARE_PHASE, benchmark_spec=spec)
  ...
  events.after_phase.send(events.PREPARE_PHASE, benchmark_spec=spec)
  ...
  def _StopTraces():
    ...
    with lock:
      if traces_stopped:
        ...
        return
      events.stop_trace.send(events.RUN_PHASE, benchmark_spec=spec)
  ...
  while True:
    ...
    if benchmark_control_traces:
      if trace_start_delay:
        logging.warn('Benchmark {} controls it\'s own traces behavior and is '
      ...
      trace_start_thread = Timer(trace_start_delay, events.start_trace.send, [events.RUN_PHASE],
                                 {"benchmark_spec": spec})
      ...
      events.start_trace.send(events.RUN_PHASE, benchmark_spec=spec)
...
def RunBenchmark(spec, collector):
  ...
  Args:
    ...
    collector: The SampleCollector object to add samples to.
  """
  intel_kafka_publisher.Register()
    ...
    with spec.RedirectGlobalFlags():
      ...
      try:
        ...
        if (FLAGS.run_stage == stages.STAGES and
          ...
          events.samples_created.send(
              events.RUN_PHASE, benchmark_spec=spec, samples=end_to_end_timer.GenerateSamples())
...
def SetUpPKB():
  ...
  if FLAGS.offline_mode:
    offlineProxySvc = offline_util.OfflineProxyService()
    _InitOfflineMode(offlineProxySvc)
    return offlineProxySvc
...
def RunBenchmarks(offlineProxySvc=None):
  """Runs all benchmarks in PerfKitBenchmarker.
  ...
  if FLAGS.intel_publish and FLAGS.intel_publisher_s3_archive_bucket_url:
    intel_publisher.ArchiveToS3(vm_util.GetTempDir(), FLAGS.run_uri + '.zip')
  ...
  if offlineProxySvc:
    logging.info("Shutting down offline proxy.")
    _CleanupOfflineMode(offlineProxySvc)
...
def Main():
  ...
  if FLAGS.helpmatchmd:
    ...
    return 0
  CheckVersionFlag()
  offlineProxySvc = SetUpPKB()
  return RunBenchmarks(offlineProxySvc)
