...
# Additional commands to run in case the above commands fails on a particular cloud image.
# Note: Captialization of packages matter.
_EPEL_ALTERNATIVE_CMDS = {
    os_types.CENTOS8: 'sudo dnf config-manager --set-enabled powertools'
}
...
def YumInstall(vm):
  ...
  if vm.OS_TYPE in _EPEL_CMDS:
    if vm.OS_TYPE in _EPEL_ALTERNATIVE_CMDS:
      try:
        vm.RemoteCommand(_EPEL_CMDS[vm.OS_TYPE])
      except (errors.VirtualMachine.RemoteCommandError):
        logging.info("Additional EPEL command failed, but there is an alternative to try.")
        vm.RemoteCommand(_EPEL_ALTERNATIVE_CMDS[vm.OS_TYPE])
    else:
      vm.RemoteCommand(_EPEL_CMDS[vm.OS_TYPE])
