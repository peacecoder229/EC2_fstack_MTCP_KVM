...
def _Uninstall(vm):
  ...
  vm.RemoteCommand('sudo rm -rf {}'.format(MEMTIER_DIR))


def YumUninstall(vm):
  """Uninstalls the memtier package on the VM."""
  vm.RemoteCommand('cd {0} && sudo make uninstall'.format(LIBEVENT_DIR))
  vm.RemoteCommand('sudo rm -rf {}'.format(LIBEVENT_DIR))
  vm.RemoteCommand('rm -rf {}'.format(os.path.join(linux_packages.INSTALL_DIR, LIBEVENT_TAR)))
  _Uninstall(vm)
