...
from perfkitbenchmarker.linux_packages import maven, openjdk
import six
from six.moves import filter
from six.moves import range
from six.moves import zip

YCSB_JAVA_COMPATIBLE_VERSION = 8
...
def _Install(vm):
  ...
  vm.Install('openjdk')
  java_version = openjdk.CheckJavaMajorVersion(vm)
  if java_version > YCSB_JAVA_COMPATIBLE_VERSION:
    raise errors.Setup.InvalidSetupError(f"Major OpenJDK version installed ({java_version}) is above 8. "
                                         f"It's incompatible with YCSB.")
...
class YCSBExecutor(object):
  ...
  def _BuildCommand(self, command_name, parameter_files=None, **kwargs):
    ...
    command = ['python2', YCSB_EXE, command_name, self.database]
      ...
      for client_count, target_qps_per_vm in _GetThreadsQpsPerLoaderList():
        ...
        def _DoRunStairCaseLoad(client_count,
          ...
          if target_qps_per_vm:
            ...
            if 'target' in parameters and parameters['target'] == 0:
