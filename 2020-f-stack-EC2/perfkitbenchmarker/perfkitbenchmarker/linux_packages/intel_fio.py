...
flags.DEFINE_string('intel_fio_version', '3.18',
                    'Flexible I/O version that will be checked out and built.')
flags.DEFINE_string('intel_fio_repository', 'https://github.com/axboe/fio.git',
                    'Repository used to pull the FIO benchmark source code')
...
def _Install(vm):
  ...
  GIT_REPO = FLAGS.intel_fio_repository
  GIT_TAG = 'fio-{0}'.format(FLAGS.intel_fio_version)
  logging.info('Checking the os distro: {}'.format(vm.OS_TYPE))
  if vm.OS_TYPE == 'clear':
    vm.RemoteCommand('sudo pip2 install pandas numpy')
  elif vm.OS_TYPE == 'ubuntu2004':
  ...
  if flags.FLAGS.intel_fio_hist_log:
