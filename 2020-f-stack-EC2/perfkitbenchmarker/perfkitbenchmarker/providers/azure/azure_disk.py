...
# Ds v4/D v4 and Es v4/E v4 instance series comes w/o temporary storage associated, hereby the attached storage will start from "b" suffix (e.g. /dev/sdb).
NO_TMP_REGEX = [
    r'Standard_D(\d*)s?_v4',
    r'Standard_E(\d*)s?_v4',
]
...
def _GenerateDrivePathSuffixes(machine_type):
  ...
  # We want to start at 'c', so fast-forward the iterator by two, unless running on an instance with no temporary storage.
  tmp_storage = True
  for regex in NO_TMP_REGEX:
    instance_series = re.compile(regex)
    if instance_series.search(machine_type):
      tmp_storage = False
  if tmp_storage:
    next(products)
  next(products)
...
class AzureDisk(disk.BaseDisk):
  ...
  def GetDevicePath(self):
    ...
    if self.disk_type == disk.LOCAL:
      ...
      # If using local, the temp disk always has a formatted partition
      return '/dev/sdb1'
    else:
      try:
        return '/dev/sd{}'.format(remote_drive_path_suffixes[self.lun])
      except IndexError:
