...
class AzureVirtualMachine(
  ...
  def FormatDisk(self, device_path, disk_type=None):
    ...
    # Attempts to modify the partition table results in an error for the temp disk
    # This is why we are overriding linux_virtual_machine for Azure
    if disk.NFS == disk_type:
      return
    if disk.SMB == disk_type:
      return
    # Attempt to umount this disk if it's mounted
    find_mnt = "mount -l|grep {}|cut -d' ' -f3".format(device_path)
    tmp_mnt = self.RemoteHostCommand(find_mnt)[0].rstrip('\n')
    if tmp_mnt:
      # We should be able to mount temp to scratch, even if this fails
      self.RemoteHostCommand("sudo umount {}".format(tmp_mnt), ignore_failure=True)
    if (disk_type != disk.LOCAL) or azure_disk.LocalDriveIsNvme(self.machine_type):
      # Format if not the preformatted local temp provided by Azure
      fmt_cmd = "sudo mke2fs -F -E lazy_itable_init=0,discard -O ^has_journal -t ext4 -b 4096 {}"\
                .format(device_path)
  ...
  def CreateAdditionalPrivateIpAddresses(self):
    ...
    if self.additional_private_ip_addresses_count:
      create_ip_cmd = "az network nic ip-config create " \
                      "--nic-name {0} " \
                      "--resource-group {1} " \
                      "--name ipconf{{0}}" \
                      .format(self.nic.name, self.resource_group.name)
      additional_private_ips = []
      for i in range(self.additional_private_ip_addresses_count):
        out, _ = vm_util.IssueRetryableCommand(create_ip_cmd.format(i).split())
        private_address = json.loads(out)["privateIpAddress"]
        additional_private_ips.append(private_address)
      self.additional_private_ip_addresses = additional_private_ips
      vm_network = ipaddress.ip_network(unicode(self.network.subnet.vnet.address_space))
      netmask = str(vm_network.netmask)
      gateway = str(vm_network.network_address + 1)
...
class Ubuntu1910BasedAzureVirtualMachine(AzureVirtualMachine,
                                         linux_virtual_machine.Ubuntu1910Mixin):
  IMAGE_URN = 'Canonical:UbuntuServer:19.10-DAILY:latest'
