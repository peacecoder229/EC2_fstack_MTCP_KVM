...
class GceVirtualMachine(virtual_machine.BaseVirtualMachine):
  ...
  def CreateAdditionalPrivateIpAddresses(self):
    """Add additional ip addresses to the VM

    Use --additional_private_addresses_count to specify the number of the new IP addresses that you
    want to assign to the VM.

    The new IP addresses will be present in the `additional_private_ip_addresses` field of the VM.
    """
    if self.additional_private_ip_addresses_count:
      region = util.GetRegionFromZone(self.zone)
      out, _, _ = vm_util.IssueCommand('gcloud compute networks subnets describe {0} '
                                       '--quiet --project {1} '
                                       '--region {2} '
                                       '--format json'
                                       .format(self.network.network_resource.name, self.project, region).split())
      addr_range = json.loads(out.strip())["ipCidrRange"]
      subnet_cidr = ipaddress.ip_network(unicode(addr_range))
      logging.info(subnet_cidr)
      out, _, _ = vm_util.IssueCommand('gcloud compute instances list '
                                       '--quiet --project {0} '
                                       '--filter=zone:{1} '
                                       '--filter=networkInterfaces[0].subnetwork:{2} '
                                       '--format json '
                                       .format(self.project, self.zone, self.network.network_resource.name).split())
      instances_json = json.loads(out)
      vms_ips = [str(instance["networkInterfaces"][0]["networkIP"]) for instance in instances_json]
      # Divide the network in equal sized subnets,
      # the netmask difference betweem the primary network and the subnets must be at least 2
      prefix_len = int(subnet_cidr.prefixlen) + max(2, int(math.ceil(math.log(len(vms_ips), 2))))
      alias_mask = "/{0}".format(prefix_len)
      out, _, _ = vm_util.IssueCommand('gcloud compute instances network-interfaces update {0} '
                                       '--quiet --project {1} '
                                       '--zone {2} '
                                       '--aliases {3}'
                                       .format(self.name, self.project, self.zone, alias_mask).split())
      out, _, _ = vm_util.IssueCommand("gcloud compute instances describe {0} "
                                       "--quiet --project {1} "
                                       "--zone={2} "
                                       "--format json"
                                       .format(self.name, self.project, self.zone).split())
      alias_range_str = json.loads(out)["networkInterfaces"][0]["aliasIpRanges"][0]["ipCidrRange"]
      alias_range = ipaddress.ip_network(unicode(alias_range_str))
      additional_private_ips = []
      valid_ips_found = 0
      for ip in alias_range.hosts():
        ip_str = str(ip)
        if valid_ips_found == self.additional_private_ip_addresses_count:
          break
        if ip_str not in vms_ips:
          additional_private_ips.append(ip_str)
          valid_ips_found += 1

      self.additional_private_ip_addresses = additional_private_ips
      netmask = "255.255.255.255"
      gateway = str(subnet_cidr.network_address + 1)
      self._AssignAdditionalPrivateIpAddresses(netmask, gateway)
...
class Ubuntu1910BasedGceVirtualMachine(
    BaseLinuxGceVirtualMachine, linux_vm.Ubuntu1910Mixin):
  DEFAULT_IMAGE_FAMILY = 'ubuntu-1910'
  DEFAULT_IMAGE_PROJECT = 'ubuntu-os-cloud'
