...
class KubernetesVirtualMachine(virtual_machine.BaseVirtualMachine):
  ...
  def __init__(self, vm_spec):
    ...
    self.os_type = FLAGS.os_type
    self.group_name = vm_spec.group_name
  ...
  def _Create(self):
    ...
    self._RunPodInitializationCommand()
  ...
  def _RunPodInitializationCommand(self):
    if self.CONTAINER_COMMAND:
      logging.info('Running initializing command inside the POD.')
      init_cmd = [FLAGS.kubectl, '--kubeconfig=%s' % FLAGS.kubeconfig, 'exec', '-i',
                  self.name, "--"] + self.CONTAINER_COMMAND
      vm_util.IssueCommand(init_cmd)
      logging.info('Finished running initializing command inside the POD.')
...
class DebianBasedKubernetesVirtualMachine(
  ...
  def RemoteHostCopy(self, file_path, remote_path='', copy_to=True):
    ...
    if copy_to:
      ...
      # kubectl cp only works with file2file or dir2dir. File2dir is not possible
      # for more details: kubectl cp --help
      if os.path.isdir(file_path):
        remote_file_name = os.path.basename(remote_path)
        file_path += "/{0}".format(remote_file_name)
...
def _install_sudo_command():
  ...
  # the container does not exit. On older Linux distros it was enough
  # to remove 'Defaults env_reset', but at least under Ubuntu 18.04 one
  # has to disable the option explicitly.
