...
class AwsVirtualMachine(virtual_machine.BaseVirtualMachine):
  ...
  def CreateAdditionalPrivateIpAddresses(self):
    ...
    if self.additional_private_ip_addresses_count:
      region = self.region
      instance_id = self.id
      describe_interface_cmd = "aws ec2 --region={0} describe-network-interfaces " \
                               "--filters Name=attachment.instance-id,Values={1} " \
                               "Name=association.public-ip,Values={2}" \
                               .format(region, instance_id, self.ip_address).split()
      out, _, _ = vm_util.IssueCommand(describe_interface_cmd)
      ifaces = json.loads(str(out.strip()))
      if ifaces:
        iface_id = ifaces["NetworkInterfaces"][0]["NetworkInterfaceId"]
        assign_ip_cmd = "aws ec2 --region={0} assign-private-ip-addresses " \
                        "--secondary-private-ip-address-count={1} --network-interface-id={2}" \
                        .format(region, self.additional_private_ip_addresses_count, iface_id).split()
        out, _ = vm_util.IssueRetryableCommand(assign_ip_cmd)
        out, _, _ = vm_util.IssueCommand(describe_interface_cmd)
        ifaces = json.loads(str(out.strip()))
        iface = ifaces["NetworkInterfaces"][0]
        self.additional_private_ip_addresses = [str(addr["PrivateIpAddress"])
                                                for addr in iface["PrivateIpAddresses"]
                                                if not addr["Primary"]]
        cidr = self.network.subnet.cidr_block
        vm_network = ipaddress.ip_network(unicode(cidr))
        netmask = str(vm_network.netmask)
        gateway = str(ipaddress.ip_address(int(vm_network.network_address) + 1))
        # os specific
...
class Ubuntu1910BasedAwsVirtualMachine(AwsVirtualMachine,
                                       linux_virtual_machine.Ubuntu1910Mixin):
  IMAGE_NAME_FILTER = 'ubuntu/images/*/ubuntu-eoan-19.10-*64-server-20*'
