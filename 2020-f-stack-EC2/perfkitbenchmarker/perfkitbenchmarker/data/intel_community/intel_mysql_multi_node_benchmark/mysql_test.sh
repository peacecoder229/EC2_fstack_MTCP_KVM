...
echo "mysql -h$host -u$sqluser -p123456 -P$port -e 'select version();'"
mysql -h$host -u$sqluser -p123456 -P$port -e "select version();" &>/dev/null
result=$?
while [ $result -ne 0 ]
do
	echo "Result=$result Waiting for MySQL start up"
	mysql -h$host -u$sqluser -p$mysqlpasswd -P$port -e "select version();" &>/dev/null
	result=$?
	sleep 1
done
...
do
  ...
  echo "$cmd run | tee logs/$logname"
  $cmd run |sudo tee logs/$logname
  echo "wait"
done
