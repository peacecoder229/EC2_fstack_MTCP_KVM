...
var duration = flag.Uint("t", 60, "Duration of each benchmark in seconds")
...
var goBenchmarks = []struct {
	...
	{"compress_gzip_compression_Twain_(-8)",
		func() func() {
			var b bytes.Buffer
	...
	{"compress_gzip_decompression_Twain_(-d)",
		func() func() {
			var b bytes.Buffer
			w, _ := gzip.NewWriterLevel(&b, 8)
			w.Write(textTwain)
	...
	{"compress_gzip_compression_digits_(-8)",
		func() func() {
			var b bytes.Buffer
			w, _ := gzip.NewWriterLevel(&b, 8)
	...
	{"compress_gzip_decompression_digits",
		func() func() {
			var b bytes.Buffer
			w, _ := gzip.NewWriterLevel(&b, 8)
			w.Write(textE)
...
func main() {
	...
	for _, b := range goBenchmarks {
		...
		rawResult := bench(b.benc, *nproc)
		fmt.Print(b.report(rawResult))
